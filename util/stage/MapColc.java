...
public abstract class MapColc extends Data implements IndexContainer.SingleIC<StageMap> {
	...
	public static class DefMapColc extends MapColc {
							...
							case 11:
								if (!parameter.isEmpty()) {
									...
									if (map.lim.isEmpty() || map.lim.get(map.lim.size() - 1).stageLimit == null)
										map.lim.add(new Limit(new StageLimit()));
									StageLimit slim = map.lim.get(map.lim.size() - 1).stageLimit;
									slim.costIncreaseMode = increaseMode;
									slim.costIncreaseValue = increaseValue;
									slim.costMaxIncreaseValue = increaseMax;
								}
                            default:
		...
		private DefMapColc() {
            ...
            add(17, id -> new StageMap(id, abbr + "2_2_Z.csv", 3)).names.put("CotC 3 Zombie");
            add(18, id -> new StageMap(id, abbr + "2_2_Invasion_Z.csv", 2)).names.put("CotC 3 Invasion Zombie");
	...
	public static class PackMapColc extends MapColc {
		...
		@Deprecated
		public PackMapColc(UserPack pack, ISStream is) {
			this.pack = pack;
			UserProfile.getRegister(REG_MAPCOLC, MapColc.class).put(pack.getSID(), this);
			int ver = getVer(is.nextString());
			if (ver < 301)
				return;
			is.nextString();//Old discarded name variable
			...
			if (ver >= 308) {
				...
				for (int i = 0; i < n; i++) {
					int ind = is.nextInt();
					int m = is.nextInt();
					CharaGroup cg = new CharaGroup(pack.getID(CharaGroup.class, ind));
					for (int j = 0; j < m; j++) {
						Unit u = (Unit) Identifier.parseInt(is.nextInt(), Unit.class).get();
						if (u != null)
							cg.fset.addAll(Arrays.asList(u.forms));
					}
					pack.groups.set(ind, cg);
				}
				is.nextInt();
				n = is.nextInt();
			}
			for (int i = 0; i < n; i++) {
				...
				int m = is.nextInt();
				for (int j = 0; j < m; j++)
					sm.add(id -> Data.err(() -> new Stage(pack, id, is.subStream())));
				if (ver < 307)
					continue;
				m = is.nextInt();
				for (int j = 0; j < m; j++)
					sm.lim.add(new Limit.PackLimit(pack, is, ver));
			}
		}
